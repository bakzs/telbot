...
#INITIALISATIONS REQUIRED FROM HOST OF THE SCRIPT.
'''
The host running this script needs to key in their driver path and screenshot path
for the program to run. Geckodriver has to be downloaded'''

token = '353987109:AAFa3AQZ0fVQ6qTS9J3ivCNC8y50SIlXbGg'

bot = telepot.Bot(token)

driver_path = r"/Users/bakzs/Desktop/MyApps/Python Telegram Bot/geckodriver"



'''Path of screenshot needed to send to the user.
   Note:
   1. For hosts that are finding their paths for screenshot.png, you will need to run this program for the first time
    to get the file path.
   2. The script that is running needs to be in the same folder as screenshot.png'''

screenshot_file_path = r'/Users/bakzs/Desktop/MyApps/Python Telegram Bot/Screenshot_test.py/screenshot.png'
...
def define_adjustment(kilometers_float):
    # Multipliers of the kilometers_float are used to produce a more accurate +- buffer distance
...
def send_image(chat_id):
    ...
    files = {'photo': open(screenshot_file_path, 'rb')}
...
def screenshot_image():
    ...
    # can be removed to prevent clogging of popup images
    im.show()  
...
def communicator(msg):              
    ...
    if content_type == 'text':
        ...
        #When the inputs are texts that are not numbers, the float conversion will result in a ValueError. Hence except will be executed.
        except ValueError:
            # Methods used are to rid off whitespaces and capital letters to get a more uniform variable
            command = msg['text'].strip().lower()
            #calories, weight and location are changed to given value 0 so that it will be defined in order to return a value to the main function
            calories = 0
            weight = 0
            location = 0
            if command == '/start':
                actkeyboard = ReplyKeyboardMarkup(keyboard=[
                    [KeyboardButton(text='Yeah let\'s run!')],[KeyboardButton(text='Imma skywalker')],
            ...
            elif command == "yeah let's run!" or command == 'imma skywalker':
      ...
      #When input is not text, the variables reset. Starting over again to get proper inputs.
      #And so that return values are defined.
